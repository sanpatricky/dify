...
from services.account_ports import AccountEmailUnitOfWork, UnitOfWorkFactory
...
class AccountChangeEmailService:
    def __init__(
        self,
        *,
        unit_of_work: UnitOfWorkFactory[AccountEmailUnitOfWork],
    ...
    def reset(
        ...
        *,
        ...
        with self._unit_of_work() as unit_of_work:
            account = unit_of_work.accounts.get(context.account_id)
            if account is None or account.email.lower() != token_data.old_email.lower():
                raise AccountNotFoundError
            if unit_of_work.accounts.email_exists(normalized_new_email):
                raise AccountEmailAlreadyInUseError
            updated_account = unit_of_work.accounts.update_email(context.account_id, normalized_new_email)
            if updated_account is None:
                raise AccountNotFoundError
            unit_of_work.integrations.delete_for_account(context.account_id)
            unit_of_work.commit()
    ...
    def ensure_available(self, email: str) -> None:
        ...
        if self._email_policy.is_frozen(normalized_email):
            raise AccountEmailFrozenError
        with self._unit_of_work() as unit_of_work:
            if unit_of_work.accounts.email_exists(normalized_email):
                raise AccountEmailAlreadyInUseError
